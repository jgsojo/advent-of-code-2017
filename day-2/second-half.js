...
function getChecksum(input) {
	let result = 0;
	input.forEach(row => {
		result += calculateEvenlyDivisionValue(row);
	});
	return result;
}

/**
 * Calculates the division between the only two values that are evenly divisible in the given row
 * 
 * @param {String} row row with values separated by spaces
 * @returns {Number} difference
 */
function calculateEvenlyDivisionValue(row) {
    let numbers = getNumberList(row);
    let evenlyDivisibleValues = getEvenlyDivisibleValues(numbers);
    return evenlyDivisibleValues.dividend / evenlyDivisibleValues.divisor;
}
...
/**
 * Returns an object with the only two values that are evenly divisible in the given row
 * 
 * @param {Array[Number]} numbers 
 * @returns {Object}
 */
function getEvenlyDivisibleValues(numbers) {
    let found = false;
    let index = 0;
    let result = {
        dividend: null,
    ...
    while (!found) {
        let divisor = getEvenlyDivisor(index, numbers);
        if (divisor) {
            found = true;
            result.dividend = numbers[index];
...
/**
 * Returns the value from the array which divides evenly the number in the given index position, undefined otherwise
 * 
 * @param {Number} index
 * @param {Array[Number]} numbers 
 * @returns {Number}
 */
function getEvenlyDivisor(index, numbers) {
    let dividend = numbers[index];
    let possibleDivisors = numbers.slice();
    possibleDivisors.splice(index, 1);
    return possibleDivisors.find(divisor => isEvenDivision(dividend, divisor));
}

/**
 * Returns whether the division between the given values is even
 * 
...
function isEvenDivision(dividend, divisor) {
    ...
    return result > 0 ? result === Math.floor(result) : result === Math.ceil(result);
}

console.log(getChecksum(input)); // 261